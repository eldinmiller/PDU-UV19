...
void readInputsA(void);
void readInputsB(void);
...
volatile uint8_t pdu_kill_trigger = 0;
volatile uint8_t kill_counter = 0;

volatile uint8_t inputs_a = 0;
volatile uint8_t inputs_b = 0;
...
int main(void)
{
	...
	while (1) {
		if(kill_function() > 0){
			pdu_output_20a_disable(8);
			pdu_output_5a_disable(1);
			pdu_output_5a_disable(12);
		}
		else{
			pdu_output_20a_enable(8);
			pdu_output_5a_enable(1);
			pdu_output_5a_enable(12);
		}
		PORTA = outputs_20a;
